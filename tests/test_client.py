...
class TestRemoteExecution(object):
    def test_remote_execution(self, client):
        ...
        with mock.patch('servy.client.Client.PROTOCOL.Request.send') as send:
            send.return_value = content
    ...
    def test_http_exception_404(self, client):
        with mock.patch('servy.protocol.http.urllib2.urlopen') as send:
            send.side_effect = urllib2.HTTPError(str(client.dsn), 404, 'Not Found', [], io.StringIO())
    ...
    def test_http_exception_503(self, client):
        with mock.patch('servy.protocol.http.urllib2.urlopen') as send:
            ...
            fp.seek(0)
            send.side_effect = urllib2.HTTPError(str(client.dsn), 503, 'Service Unavailable', [], fp)
            with pytest.raises(servy.exc.RemoteException):
                client.fn()

    def test_http_exception_503_failed(self, client):
        with mock.patch('servy.protocol.http.urllib2.urlopen') as send:
            send.side_effect = urllib2.HTTPError(str(client.dsn), 503, 'Service Unavailable', [], io.StringIO())
            with pytest.raises(servy.exc.RemoteException):
                client.fn()

    def test_uncovered_http_exception(self, client):
        with mock.patch('servy.protocol.http.urllib2.urlopen') as send:
            send.side_effect = urllib2.HTTPError(str(client.dsn), 600, 'Magic', [], io.StringIO())
